...
# Test svyrep.design works
test_that("ard_tabulate_value.svyrep.design() works", {
  rsvy_dicho <- survey::svydesign(ids = ~1, data = mtcars, weights = ~1) |>
    survey::as.svrepdesign() |>
    suppressWarnings()
  # convert variables to factor
  rsvy_dicho$variables <- rsvy_dicho$variables |>
    dplyr::mutate(across(c("cyl", "am", "vs"), as.factor))
...
test_that("ard_dichotomous.svyrep.design() works with various input types", {
  rsvy_dicho <- survey::svydesign(ids = ~1, data = mtcars, weights = ~1) |>
    survey::as.svrepdesign() |>
    suppressWarnings()
...
test_that("ard_tabulate_value.svyrep.design() returns an error with erroneous input", {
  # value passed in is not logical should return an error
  rsvy_dicho <- survey::svydesign(ids = ~1, data = mtcars, weights = ~1) |>
    survey::as.svrepdesign() |>
    suppressWarnings()
...
test_that("ard_tabulate_value.svyrep.design() follows ard structure", {
  rsvy_dicho <- survey::svydesign(ids = ~1, data = mtcars, weights = ~1) |>
    survey::as.svrepdesign() |>
    suppressWarnings()
  rsvy_dicho$variables <- rsvy_dicho$variables |>
