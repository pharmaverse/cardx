...
test_that("ard_survey_svyttest() works for svyrep.design objects", {
  expect_error(
    ard_svyttest <-
      ard_survey_svyttest(
        rclus2,
  ...
  expect_equal(
    ...
    survey::svyttest(enroll ~ comp.imp, rclus2)[c("estimate", "p.value")],
    ignore_attr = TRUE
  )
  ...
  expect_equal(
    ...
    survey::svyttest(enroll ~ comp.imp, rclus2) |>
      confint(level = 0.9) |>
      as.list(),
    ignore_attr = TRUE
  )
...
test_that("ard_survey_svyttest() messaging for survey.design objects", {
  expect_error(
    ard_svyttest <-
      ard_survey_svyttest(
        dclus2,
...
test_that("ard_survey_svyttest() messaging for svyrep.design objects", {
  expect_error(
    ard_svyttest <-
      ard_survey_svyttest(
        rclus2,
...
test_that("ard_survey_svyttest() follows ard structure for survey.design objects", {
  expect_silent(
    ard_survey_svyttest(
      dclus2,
      variable = enroll,
...
test_that("ard_survey_svyttest() follows ard structure for svyrep.design objects", {
  expect_silent(
    ard_survey_svyttest(
      rclus2,
      variable = enroll,
