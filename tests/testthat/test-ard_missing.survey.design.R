...
test_that("ard_missing.svyrep.design() works", {
  svy_titanic <- survey::svydesign(~1, data = as.data.frame(Titanic), weights = ~Freq)
  ...
  # without by
  expect_error(
    ard_svy_missing <- ard_missing(rep_titanic, variables = c(Class, Age), by = NULL),
    NA
  )
  expect_invisible(cards::check_ard_structure(ard_svy_missing, method = FALSE))
  ...
  # without by, single var
  expect_error(
    ard_svy_missing <- ard_missing(rep_titanic, variables = Class, by = NULL),
    NA
  )
  expect_invisible(cards::check_ard_structure(ard_svy_missing, method = FALSE))
  ...
  # with by, single var
  expect_error(
    ard_svy_missing <- ard_missing(rep_titanic, variables = Class, by = Survived),
    NA
  )
  expect_invisible(cards::check_ard_structure(ard_svy_missing, method = FALSE))
  ...
  # with by
  expect_error(
    ard_svy_missing <- ard_missing(rep_titanic, variables = c(Class, Age), by = Survived),
    NA
  )
  expect_invisible(cards::check_ard_structure(ard_svy_missing, method = FALSE))
...
test_that("ard_missing.svyrep.design() follows ard structure", {
  svy_titanic <- survey::svydesign(~1, data = as.data.frame(Titanic), weights = ~Freq) |>
...
# testing bug reported here: https://github.com/ddsjoberg/gtsummary/issues/2092
test_that("ard_missing.svyrep.design() works on design columns", {
  expect_equal(
    suppressWarnings(
      dplyr::tribble(
        ~region, ~year,          ~weights,
