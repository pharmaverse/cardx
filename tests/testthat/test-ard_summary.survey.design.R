...
test_that("unstratified ard_summary.svyrep.design() works", {
  ...
  expect_equal(
    cards::get_ard_statistics(ard_uni_svy_cont, stat_name %in% "mean") |> unlist(),
    survey::svymean(x = ~api00, rclus1, na.rm = TRUE)[1] |> unlist(),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard_uni_svy_cont, stat_name %in% "median") |> unlist(),
    survey::svyquantile(x = ~api00, rclus1, na.rm = TRUE, quantiles = 0.5)[[1]][1] |> unlist(),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard_uni_svy_cont, stat_name %in% "min") |> unlist(),
    rclus1$variables$api00 |> min(na.rm = TRUE),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard_uni_svy_cont, stat_name %in% "max") |> unlist(),
    rclus1$variables$api00 |> max(na.rm = TRUE),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard_uni_svy_cont, stat_name %in% "var") |> unlist(),
    survey::svyvar(x = ~api00, rclus1, na.rm = TRUE)[1] |> unlist(),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard_uni_svy_cont, stat_name %in% "sd") |> unlist(),
    survey::svyvar(x = ~api00, rclus1, na.rm = TRUE)[1] |> unlist() |> sqrt(),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard_uni_svy_cont, stat_name %in% "mean.std.error") |> unlist(),
    survey::svymean(x = ~api00, rclus1, na.rm = TRUE) |> survey::SE() |> unlist(),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard_uni_svy_cont, stat_name %in% "deff") |> unlist(),
    survey::svymean(x = ~api00, rclus1, na.rm = TRUE, deff = TRUE) |>
      as.data.frame() |>
      dplyr::pull(deff),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard_uni_svy_cont, stat_name %in% "p75") |> unlist(),
    survey::svyquantile(x = ~api00, rclus1, na.rm = TRUE, quantiles = 0.75)[[1]][1] |> unlist(),
    ignore_attr = TRUE
  )
...
test_that("stratified ard_summary.svyrep.design() works", {
  expect_error(
    ard_svy_cont <-
      ard_summary(
        rclus1,
  ...
  expect_equal(
    ...
    survey::svyby(~api00, by = ~both, rclus1, FUN = survey::svymean, na.rm = TRUE) %>%
      {
        dplyr::pull(., api00) |>
          as.list() |>
          set_names(rownames(.))
  ...
  expect_equal(
    ...
    survey::svyby(~api00, by = ~both, rclus1, FUN = survey::svyquantile, na.rm = TRUE, quantiles = 0.5) %>%
      {
        dplyr::pull(., api00) |>
          as.list() |>
          set_names(rownames(.))
  ...
  expect_equal(
    ...
    survey::svyby(~api00, by = ~both, rclus1, FUN = survey::svyvar, na.rm = TRUE) %>%
      {
        dplyr::pull(., V1) |>
          as.list() |>
          set_names(rownames(.))
  ...
  expect_equal(
    ...
    survey::svyby(~api00, by = ~both, rclus1, FUN = survey::svymean, na.rm = TRUE, deff = TRUE) %>%
      {
        dplyr::pull(., DEff.api00) |>
          as.list() |>
          set_names(rownames(.))
  ...
  expect_equal(
    ...
    survey::svyby(~api00, by = ~both, rclus1, FUN = survey::svyquantile, na.rm = TRUE, quantiles = 0.75) %>%
      {
        dplyr::pull(., api00) |>
          as.list() |>
          set_names(rownames(.))
...
test_that("ard_summary.svyrep.design() NA handling", {
  expect_error(
    ard_uni_NA_svy_cont <-
      ard_summary(
        rclus1_na,
...
test_that("ard_summary.svyrep.design() error handling", {
  # passing a character vector (some results are still calculable...i don't think we need to "fix" these)
  # and these "results" may vary across systems (all are nonsense), so just check
  # that code runs without error
  expect_error(
...
test_that("ard_summary.svyrep.design(fmt_fun)", {
  expect_snapshot(
    ard_summary(
      rclus1,
      variables = c(api99, api00),
...
test_that("ard_summary.svyrep.design(stat_label)", {
  expect_snapshot(
    ard_summary(
      rclus1,
      variables = c(api00, api99),
...
test_that("ard_summary.survey.design(by) unobserved levels/combinations", {
  ...
  expect_setequal(
    levels(dclus1_fct$variables$both),
    ard_summary(
      dclus1_fct,
      variables = api00,
      by = both,
  ...
  with(dclus1_fct$variables, table(stype, awards))
  expect_equal(
    ard_summary(
      dclus1_fct,
      variables = api00,
...
test_that("ard_summary.svyrep.design(by) unobserved levels/combinations", {
  # The 'Neither' level is never observed, but included in the table
  expect_setequal(
    levels(rclus1_fct$variables$both),
    ard_summary(
      rclus1_fct,
      variables = api00,
      by = both,
  ...
  # stype="E" is not observed with awards="No", but it should still appear in table
  with(rclus1_fct$variables, table(stype, awards))
  expect_equal(
    ard_summary(
      rclus1_fct,
      variables = api00,
...
# - test if function parameters can be used as variable names without error
test_that("ard_summary.svyrep.design() works when using generic names ", {
  rclus2 <- rclus1
  rclus2$variables <- rclus1$variables %>%
    dplyr::rename("variable_level" = cds, "variable" = stype, "median" = dnum, "p25" = snum)


  expect_equal(
    ard_summary(rclus1, variables = c(cds, stype), by = dnum) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(variable_level, variable), by = median) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(dnum, snum), by = cds) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(median, p25), by = variable_level) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(dnum, snum), by = stype) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(median, p25), by = variable) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(dnum, cds), by = snum) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(median, variable_level), by = p25) |> dplyr::select(stat)
  )
  ...
  expect_equal(
    ard_summary(rclus1, variables = c(cds, stype), by = dnum) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(by, statistic), by = weights) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(stype, dnum), by = cds) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(statistic, weights), by = by) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(cds, dnum), by = stype) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(by, weights), by = statistic) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(cds, stype), by = snum) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(by, statistic), by = p75) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(cds, snum), by = stype) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(by, p75), by = statistic) |> dplyr::select(stat)
  )
  ...
  expect_equal(
    ard_summary(rclus1, variables = c(cds, stype), by = dnum) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(mean, sd), by = var) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(stype, dnum), by = cds) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(sd, var), by = mean) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(cds, dnum), by = stype) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(mean, var), by = sd) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(cds, stype), by = snum) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(mean, sd), by = sum) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(cds, snum), by = stype) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(mean, sum), by = sd) |> dplyr::select(stat)
  )
  ...
  expect_equal(
    ard_summary(rclus1, variables = c(cds, stype), by = dnum) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(deff, min), by = max) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(stype, dnum), by = cds) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(min, max), by = deff) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(cds, dnum), by = stype) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(deff, max), by = min) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(cds, stype), by = snum) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(deff, min), by = mean.std.error) |> dplyr::select(stat)
  )

  expect_equal(
    ard_summary(rclus1, variables = c(cds, snum), by = stype) |> dplyr::select(stat),
    ard_summary(rclus2, variables = c(deff, mean.std.error), by = min) |> dplyr::select(stat)
  )
})
...
test_that("ard_summary.svyrep.design() follows ard structure", {
  expect_silent(
    ard_summary(rclus1, variables = c(cds, stype), by = snum) |>
      cards::check_ard_structure(method = FALSE)
  )
})
...
test_that("min/max do not include zero-weight rows for svyrep.design (#352)", {
  # A zero-weight observation contributes to no point estimate; the
  # extremes must not come from it either.
  df <- data.frame(
    x = c(1, 2, 3, 4, 5, 7, 9, 999),
  ...
  rdes <- survey::svydesign(ids = ~1, weights = ~w, data = df) |>
    survey::as.svrepdesign()
  ard <- ard_summary(rdes, variables = x, statistic = ~ c("min", "max"))
  expect_equal(
    cards::get_ard_statistics(ard, stat_name %in% "min") |> unlist(),
    2,
    ignore_attr = TRUE
...
test_that("ard_summary() works for designs built with survey::svrepdesign()", {
  # Replicate designs often arrive as a data set with replicate weight columns
  # supplied directly, rather than derived from a linearized design with
  # `as.svrepdesign()`. `min`/`max` read the sampling weights, so those must be
  # taken from the design's `pweights` and not from the replicate weights.
  df <- data.frame(
    x = c(1, 2, 3, 4, 5, 7, 9, 999),
    w = c(0, 1, 1.5, 2, 1, 0.5, 2, 0)
  )
  # replicate weight columns, relative to the sampling weights in `w`
  repweights <- cbind(
    c(0, 0, 2, 2, 1, 1, 1, 0),
    c(0, 2, 0, 2, 1, 1, 1, 0),
    c(0, 1, 1, 1, 0, 0, 2, 0),
  ...
  expect_error(
    ard <- ard_summary(
      rdes,
      variables = x,
      statistic = ~ c("mean", "min", "max", "sum")
    ),
    NA
  )
  expect_invisible(cards::check_ard_structure(ard))

  # the zero-weight rows contribute to no point estimate, and must not supply
  # the extremes either (#352)
  expect_equal(
    cards::get_ard_statistics(ard, stat_name %in% "min") |> unlist(),
    2,
    ignore_attr = TRUE
  ...
  # the point estimates are calculated from the sampling weights passed to
  # `weights=`, not from the replicate weights
  expect_equal(
    cards::get_ard_statistics(ard, stat_name %in% "mean") |> unlist(),
    stats::weighted.mean(df$x, df$w),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard, stat_name %in% "sum") |> unlist(),
    sum(df$x * df$w),
    ignore_attr = TRUE
  )

  # and agree with what `survey` itself reports for this design
  expect_equal(
    cards::get_ard_statistics(ard, stat_name %in% "mean") |> unlist(),
    survey::svymean(x = ~x, rdes, na.rm = TRUE)[1] |> unlist(),
    ignore_attr = TRUE
  )
  expect_equal(
    cards::get_ard_statistics(ard, stat_name %in% "sum") |> unlist(),
    survey::svytotal(x = ~x, rdes, na.rm = TRUE)[1] |> unlist(),
    ignore_attr = TRUE
  )
})
