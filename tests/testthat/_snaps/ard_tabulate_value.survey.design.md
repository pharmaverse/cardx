...
# ard_tabulate_value.svyrep.design() returns an error with erroneous input

    Code
      ard_tabulate_value(rsvy_dicho, by = vs, variables = c(cyl, am), value = list(
        cyl = 4), denominator = "row")
    Condition
      Error in `ard_tabulate_value.survey.design()`:
      ! Error in argument `value` for variable "cyl".
      i A value of 4 was passed, but must be one of TRUE and FALSE.
      i To summarize this value, make the column a factor and include 4 as a level.
...
---
    ...
    Code
      ard_tabulate_value(rsvy_dicho, by = cyl, variables = c(vs, am), value = list(
        vs = 4), denominator = "row")
    Condition
      Error in `ard_tabulate_value.survey.design()`:
      ! Error in argument `value` for variable "vs".
      i A value of 4 was passed, but must be one of 0 and 1.
...
---
    ...
    Code
      ard_tabulate_value(rsvy_dicho, by = cyl, variables = c(vs, disp), value = list(
        disp = "turn"), denominator = "row")
    Condition
      Error in `ard_tabulate_value.survey.design()`:
      ! Error in argument `value` for variable "disp".
